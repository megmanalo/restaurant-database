...
public class SidesMenuTabSection extends MenuTabSection {

    /*
     * Creates the sides section of the menu tab
     */
    public SidesMenuTabSection(RestaurantDatabase controller) {
        super(controller);
        
        createFirstComponent("SIDES", "Side");
    ...
    protected void createListPanel() {
        ...
        for (MenuItem item : database.getMenu().getSides()) {
    ...
    protected void createNewMenuItem() {
        ...
        // TO-DO: make abstract function per section of menu tab
        if (!database.getMenu().getSides().contains(newItem)) {
            if (price >= 0) {
                database.getMenu().addItem(database.getMenu().getSides(), newItem);
            }
        }
    }
    ...
    public void deleteMenuItem() {
        // TO-DO: make abstract function per section of menu tab
        database.getMenu().removeItem(database.getMenu().getSides(), list.getSelectedValue());
